...
fn test_multiple_reports_different_content() {
    ...
    let summary_content = summary_report.generate_content(&data_refs, &config).unwrap();
    let performance_content = performance_report.generate_content(&data_refs, &config).unwrap();
