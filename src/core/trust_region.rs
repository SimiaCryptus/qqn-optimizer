...
use crate::utils::math::{compute_magnitude, dot_product, vector_add, vector_scale, vector_subtract, DifferentiableFunction};
...
impl TrustRegionOptimizer {
    ...
    pub fn new(config: TrustRegionConfig) -> Self {
        if config.verbose {
            info!("Creating Trust Region optimizer with initial radius: {}", config.initial_radius);
        }
...
impl Optimizer for TrustRegionOptimizer {
    ...
    fn step(
        ...
        if self.config.verbose {
            debug!("Trust Region step {} starting with radius: {}", 
        ...
        if self.config.verbose {
            debug!("Current function value: {:.6e}, gradient norm: {:.6e}", 
        ...
        let step = self.solve_subproblem(&gradient, self.state.hessian_approx.as_deref(), self.state.radius)?;
        ...
        // Compute trial point
        let trial_params: Vec<Tensor> = params.iter()
            .zip(step.iter())
            .map(|(p, s)| p.add(s))
            .collect::<CandleResult<Vec<_>>>()?;
        ...
        if self.config.verbose {
            debug!("Step norm: {:.6e}, model reduction: {:.6e}, actual reduction: {:.6e}, rho: {:.6e}",
        ...
        self.state.prev_function_value = Some(if step_accepted { trial_value } else { current_value });
        
        // Create metadata
        let mut metadata = OptimizationMetadata::default();
        metadata.timing_info.step_duration = start_time.elapsed();
        metadata.optimizer_data.insert("trust_region_radius".to_string(), self.state.radius);
        metadata.optimizer_data.insert("gradient_norm".to_string(), grad_norm);
        metadata.optimizer_data.insert("step_norm".to_string(), step_norm);
        metadata.optimizer_data.insert("rho".to_string(), rho);
        metadata.optimizer_data.insert("step_accepted".to_string(), if step_accepted { 1.0 } else { 0.0 });
        metadata.optimizer_data.insert("consecutive_rejections".to_string(), self.state.consecutive_rejections as f64);
