...
impl UnifiedReportTestSuite {
    ...
    pub fn test_basic_functionality<R: Report>(report: &R) -> Result<()> {
        ...
        assert!(!report.description().is_empty(), "Report description should not be empty");
        
        // Test supported formats
        let formats = report.supported_formats();
        assert!(!formats.is_empty(), "Report should support at least one format");
    ...
    pub fn test_content_generation<R: Report>(report: &R) -> Result<()> {
        ...
        for format in &report.supported_formats() {
            ...
            assert!(!content.is_empty(), "Generated content should not be empty for format {:?}", format);
    ...
    pub fn test_metadata_generation<R: Report>(report: &R) -> Result<()> {
        ...
        assert!(metadata.problem_count > 0, "Should have non-zero problem count");
        assert!(metadata.optimizer_count > 0, "Should have non-zero optimizer count");
    ...
    pub fn test_all_formats<R: Report>(report: &R) -> Result<()> {
        ...
        for format in &[ReportFormat::Html, ReportFormat::Latex, ReportFormat::Markdown, ReportFormat::Csv] {
    ...
    fn test_report_formats_consistency() {
        ...
        for i in 0..values.len() {
            for j in i+1..values.len() {
                assert_ne!(values[i], values[j], "Different formats should produce different content");
            }
        }
    }
}