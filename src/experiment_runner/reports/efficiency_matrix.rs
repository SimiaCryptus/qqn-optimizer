...
impl EfficiencyMatrixReport {
    ...
    fn collect_families(&self, all_results: &[(&ProblemSpec, BenchmarkResults)]) -> (Vec<String>, Vec<String>) {
    ...
    fn calculate_efficiency_data(&self, all_results: &[(&ProblemSpec, BenchmarkResults)]) -> HashMap<(String, String), (f64, f64, usize)> {
        ...
        for optimizer_family in &optimizer_families {
            for problem_family in &problem_families {
                ...
                for (problem, results) in all_results {
                    if report_generator::get_family(&problem.get_name()) == *problem_family {
                        for result in &results.results {
                            ...
                            if result_optimizer_family == *optimizer_family && result.convergence_achieved {
                ...
                if !successful_evaluations.is_empty() {
                    let mean = successful_evaluations.iter().sum::<f64>() / successful_evaluations.len() as f64;
                    let variance = successful_evaluations
                        .iter()
                        .map(|x| (x - mean).powi(2))
                        .sum::<f64>() / successful_evaluations.len() as f64;
                    let std_dev = variance.sqrt();
                    efficiency_data.insert((optimizer_family.clone(), problem_family.clone()), (mean, std_dev, successful_evaluations.len()));
                }
            }
        }
    ...
    fn generate_html(&self, all_results: &[(&ProblemSpec, BenchmarkResults)], _config: &ReportConfig) -> anyhow::Result<String> {
        let (optimizer_families, problem_families) = self.collect_families(all_results);
...
<body>
    ...
    <table>
        ...
        for optimizer_family in &optimizer_families {
            html_content.push_str(&format!("<tr><td class=\"optimizer-name\">{}</td>", optimizer_family));
            
            for problem_family in &problem_families {
                let cell_content = if let Some((mean, std_dev, _count)) = efficiency_data.get(&(optimizer_family.clone(), problem_family.clone())) {
    ...
    fn generate_latex(&self, all_results: &[(&ProblemSpec, BenchmarkResults)], _config: &ReportConfig) -> anyhow::Result<String> {
        let (optimizer_families, problem_families) = self.collect_families(all_results);
        let efficiency_data = self.calculate_efficiency_data(all_results);
...
"#,
    ...
    fn generate_markdown(&self, all_results: &[(&ProblemSpec, BenchmarkResults)], _config: &ReportConfig) -> anyhow::Result<String> {
        let (optimizer_families, problem_families) = self.collect_families(all_results);
        let efficiency_data = self.calculate_efficiency_data(all_results);
    ...
    fn generate_csv(&self, all_results: &[(&ProblemSpec, BenchmarkResults)], _config: &ReportConfig) -> anyhow::Result<String> {
        let (optimizer_families, problem_families) = self.collect_families(all_results);
        let efficiency_data = self.calculate_efficiency_data(all_results);
...
impl Report for EfficiencyMatrixReport {
    ...
    fn export_to_file(&self, data: &[(&ProblemSpec, BenchmarkResults)], config: &ReportConfig, output_path: &Path) -> anyhow::Result<()> {
        ...
        fs::write(output_path, content).with_context(|| {
            format!("Failed to write efficiency matrix report to: {}", output_path.display())
        })?;
        Ok(())
    }
    ...
    fn validate_data(&self, data: &[(&ProblemSpec, BenchmarkResults)]) -> anyhow::Result<()> {
        ...
        let has_convergence_data = data.iter().any(|(_, results)| {
            results.results.iter().any(|r| r.convergence_achieved)
        });
        
        if !has_convergence_data {
            return Err(anyhow::anyhow!("No convergence data found in benchmark results"));
        }
    ...
    fn get_metadata(&self, data: &[(&ProblemSpec, BenchmarkResults)]) -> ReportMetadata {
        ...
        let total_successful_runs: usize = efficiency_data.values().map(|(_, _, count)| count).sum();
        
        let mut metadata = HashMap::new();
        metadata.insert("optimizer_families".to_string(), optimizer_families.len().to_string());
        metadata.insert("problem_families".to_string(), problem_families.len().to_string());
        metadata.insert("total_successful_runs".to_string(), total_successful_runs.to_string());
        metadata.insert("matrix_cells".to_string(), (optimizer_families.len() * problem_families.len()).to_string());
